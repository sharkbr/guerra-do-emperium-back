...
| **Painel de usuários do site** | área logada, botão **Usuários** — só para `login.group_id >= 99` (hoje só a `librasupremo`). Vê todas as contas e o estado de cada uma, reativa, bloqueia e suspende, e separa a trava automática de senha errada (§4.23) do castigo aplicado por gente. `site/admin.go`; o porquê de cada trava em `site/LEIAME.md` |
