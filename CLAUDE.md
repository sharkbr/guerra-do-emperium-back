...
| `src/map/clif.cpp` | **quatro** includes de `src/custom/` + **cinco** chamadas, comentadas no arquivo: `placa_de_venda_mostra`, o teto de refino nas duas pontas da janela de refino, `brilho_da_carta` no `clif_dropflooritem` (o pilar de luz e o som quando cai carta — `src/custom/brilho_da_carta.hpp`), e `roleta_do_cassino_clique` no topo do `clif_parse_CatchPet`: o `0x19f` que a captura de pet usa é também o clique na roleta dos caça-níqueis do cassino, e com giro pendente ele é da roleta e não chega ao `pet_catch_process_end`. Acréscimo, não substituição — sem giro pendente a captura segue igual (`src/custom/roleta_do_cassino.hpp`) |
...
| `src/map/pc.cpp` | **dois** includes de `src/custom/` + **duas** chamadas, comentadas no arquivo, ambas acréscimo. `estilo_de_corpo_resolve` no topo do `case LOOK_BODY2:` do `pc_changelook`, **antes** do `job_db.exists`: traduz o valor legado 0/1 que o `db/re/stylist.yml` ainda manda para o Id do trabalho do visual alternativo — sem ela a UI de estilista come o Cupom de Roupa e não muda nada (`src/custom/estilo_de_corpo.hpp`). E `rank_da_arena_reconta` no `pc_calc_pvprank`, antes do laço que acha a posição: reconta os jogadores visíveis e grava no `users_pvp`, o "M" do rank N/M da arena, que a Cópia Explosiva do fantasma desregulava até mostrar 1/43 com dois dentro (`src/custom/rank_da_arena.hpp`) |
...
- `.txt` de NPC está em CRLF, e script que leia em `rb`, corte por `\n` e junte com `\n` deixa um `\r` NO MEIO da linha ao mexer no conteúdo dela — e aí o `git diff --stat` acusa o arquivo INTEIRO, o que parece apagamento e não é
...
- Metade de uma seção de PE pode não existir em disco. A `.xdiff` deste exe tem `VirtualSize` 0x1000 e `SizeOfRawData` 0x400 — 0x800 desde o desvio da roleta (2026-09-26)
- O servidor NÃO abre a roleta da captura de pet: o `0x1a0` só mexe numa roleta já aberta, e mandado com ela fechada cai no vazio. Quem abre é o cliente, no clique do cursor sobre o monstro — e ela não para sozinha. É o `ferramentas/abre_roleta_do_cassino.py`
...
- O "M" do rank N/M da arena (`users_pvp`) é contador `++`/`--` em volta do bit do `@hide`, e a Cópia Explosiva vira o mesmo bit por fora: o número só cresce até o reinício — 1/43 com dois dentro. Recontado em `src/custom/rank_da_arena.hpp`
